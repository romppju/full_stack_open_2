...
const App = () => {
      ...
      `replace the old number with a new one?`)) {
        ...
        personService
          .update(id, changedPerson)
            ...
            .catch(error => {
              setNotification({message: `Information of ${newName} ` + 
              `has already been removed from server`, success: false})
              setTimeout(() => {
                setNotification({message: '', success: true})
              }, 5000) 
            })
  ...
  const handleDeleteOf = (id) => {
    ...
    if (window.confirm(`Delete ${personToBeDeleted.name}`)) {
      ...
      .then(returnedPersons => {
        ...
        setNotification({message: `Deleted ${personToBeDeleted.name}`, success: true}) 
        setTimeout(() => {
          setNotification({message: '', success: true})
        }, 5000)
      })
  ...
  return (
    <div>
      ...
      <h2>add a new</h2>
      <PersonForm add={addPerson} name={newName} nameChange={handleNameChange}
  ...
  return (
    <div>
      {props.persons.map(p =>
        ...
        number={p.number} handleDelete={() => props.handleDelete(p.id)}/>
      )}       
    </div>
  )
}
